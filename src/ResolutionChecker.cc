...
class ResolutionChecker : public edm::EDAnalyzer {
   public:
      explicit ResolutionChecker(const edm::ParameterSet&);
      ~ResolutionChecker();
...
//
ResolutionChecker::ResolutionChecker(const edm::ParameterSet& iConfig)
{
  // input parameters
...
void
ResolutionChecker::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   using namespace edm;
   using namespace std;
   using namespace pat;
...
void 
ResolutionChecker::beginJob(const edm::EventSetup&)
{
...
DEFINE_FWK_MODULE(ResolutionChecker);
